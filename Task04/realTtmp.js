...
const processDataFn = (db, callback) => {
    ...
    cursor.aggregate(query1).toArray(function (err, docs) {
        let ModifiedCustListCount = dataPocessFn(docs, "NotStarted", "InProgress");
        let persentage = ModifiedCustListCount * 100 / totalRoutes || 0;
    ...
    cursor.aggregate(query2).toArray(function (err, docs) {
        let ModifiedCustListCount = dataPocessFn(docs, "Complete", "InProgress");
        let persentage = ModifiedCustListCount * 100 / totalRoutes || 0;
...
function dataPocessFn(docs, State1, State2) {
    ...
    for (let item in docs) {
        ...
        var bool = _.isEqual(CustArr1.sort(), CustArr2.sort());
